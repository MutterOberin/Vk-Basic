...
std::vector<VkQueueFamilyProperties> GetPhysicalDeviceQueueFamilyProperties(VkPhysicalDevice physicalDevice)
{
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &amountOfQueueFamilies, props.data());
...
std::vector<VkLayerProperties> GetSupportedDeviceLayers(VkPhysicalDevice physicalDevice)
{
    uint32_t amountOfLayers = 0;

    vkEnumerateDeviceLayerProperties(physicalDevice, &amountOfLayers, nullptr);

    std::vector<VkLayerProperties> layers;
    layers.resize(amountOfLayers);

    vkEnumerateDeviceLayerProperties(physicalDevice, &amountOfLayers, layers.data());
...
std::vector<VkExtensionProperties> GetSupportedDeviceExtensions(VkPhysicalDevice physicalDevice)
{
    ...
    vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, &amountOfExtensions, nullptr);

    std::vector<VkExtensionProperties> extensions;
    extensions.resize(amountOfExtensions);

    vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, &amountOfExtensions, extensions.data());
...
void StartVulkan_CreateInstance()
{
    // Application Info
...
void StartVulkan_CreateSurface()
{
    ASSERT_VK(glfwCreateWindowSurface(instance, window, nullptr, &surface));
}

void StartVulkan_CreateDevice(VkPhysicalDevice physicalDevice)
{
    ...
    ASSERT_VK(vkCreateDevice(physicalDevice, &deviceCreateInfo, nullptr, &device));
}

void StartVulkan_CreateSwapchain()
{
    // Swapchain
...
void StartVulkan_CreateSwapchainImages()
{
...
void StartVulkan_CreateRenderpass()
{
...
void StartVulkan_CreatePipelineLayout()
{
    // Read Shader Files
...
void StartVulkan_CreateFramebuffers()
{
...
void StartVulkan_CreateCommandPool()
{
...
void StartVulkan_CreateCommandBuffers()
{
...
void StartVulkan_RecordCommandBuffers()
{
...
void StartVulkan()
{
    ...
    StartVulkan_CreateDevice(physicalDevices[0]);
    ...
    StartVulkan_CreateSwapchain();

    // Swapchain Images

    StartVulkan_CreateSwapchainImages();

    // Renderpass

    StartVulkan_CreateRenderpass();

    // Pipeline Layout

    StartVulkan_CreatePipelineLayout();
    
    // Framebuffers

    StartVulkan_CreateFramebuffers();

    // CommandPool amd CommandBuffers
    
    StartVulkan_CreateCommandPool();
    StartVulkan_CreateCommandBuffers();
    StartVulkan_RecordCommandBuffers();
}
